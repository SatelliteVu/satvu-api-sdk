...
The cache is automatically invalidated when the OpenAPI spec changes: spec_hash is the
spec environment plus a hash of the preprocessed spec content, so a schema change yields
a filename that does not exist yet and the examples are regenerated rather than replayed.
"""
